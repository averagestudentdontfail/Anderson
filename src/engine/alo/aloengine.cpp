 ...
 double ALOEngine::calculateCallImpl(double S, double K, double r, double q, double vol, double T) const {
     ...
     if (q <= 0.0) {
         ...
         double european_value = european_call.calculatePrice(S, K, r, q, vol, T);
         cache_[cache_key] = european_value;
         return european_value;
     }
 ...
 std::vector<double> ALOEngine::batchCalculatePut(double S, const std::vector<double>& strikes,
     ...
     // For larger batches, use SIMD with 4 options at a time if available
     if (opt::isSimdAvailable()) {
         size_t i = 0;
 ...
 std::array<double, 4> ALOEngine::calculatePut4(
     ...
     // Check if all options have the European-only condition (r <= 0.0 && r <= q)
     ...
     // If all are European, just use the Black-Scholes formula
     if (all_european) {
         mod::EuropeanPut european_put;
         return european_put.calculatePrice4(spots, strikes, rs, qs, vols, Ts);
     }
     ...
     for (size_t i = 0; i < 4; ++i) {
         ...
         if (rs[i] <= 0.0 && rs[i] <= qs[i]) {
             mod::EuropeanPut european_put;
             results[i] = european_put.calculatePrice(spots[i], strikes[i], rs[i], qs[i], vols[i], Ts[i]);
         } else {
 ...
 std::vector<double> ALOEngine::batchCalculateCall(double S, const std::vector<double>& strikes,
     ...
     // For all options, use scalar computation
     for (size_t i = 0; i < n; ++i) {

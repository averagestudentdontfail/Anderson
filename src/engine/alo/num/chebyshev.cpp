 ...
 // ===== Single-precision Chebyshev Implementation =====

ChebyshevInterpolationFloat::ChebyshevInterpolationFloat(
    size_t num_points, 
    ...
    if (num_points_ < 2) {
        throw std::invalid_argument("ChebyshevInterpolationFloat requires at least 2 points");
    }
...
void ChebyshevInterpolationFloat::initializeNodes() {
    nodes_.resize(num_points_);
...
void ChebyshevInterpolationFloat::computeCoefficients() {
    coefficients_.resize(num_points_);
    ...
    if (kind_ == FIRST_KIND) {
        ...
        for (size_t k = 0; k < num_points_; ++k) {
            float sum = 0.0f;
            for (size_t j = 0; j < num_points_; ++j) {
                sum += values_[j] * std::cos(M_PI * k * (2.0f * j + 1.0f) / (2.0f * num_points_));
            }
...
float ChebyshevInterpolationFloat::operator()(float x, bool extrapolate) const {
    // Map x to standard domain [-1, 1]
...
void ChebyshevInterpolationFloat::updateValues(const std::vector<float>& values) {
    if (values.size() != num_points_) {
        throw std::invalid_argument("Number of values must match the number of nodes");
    }
...
float ChebyshevInterpolationFloat::mapToStandardDomain(float x) const {
    return 2.0f * (x - domain_start_) / (domain_end_ - domain_start_) - 1.0f;
}

float ChebyshevInterpolationFloat::mapFromStandardDomain(float t) const {
    return 0.5f * ((domain_end_ - domain_start_) * t + domain_end_ + domain_start_);
}

float ChebyshevInterpolationFloat::evaluateChebyshev(int n, float x, ChebyshevKind kind) const {
    if (n == 0) return 1.0f;
    if (n == 1) return x;
...
SimdChebyshevInterpolationFloat::SimdChebyshevInterpolationFloat(const ChebyshevInterpolationFloat& interp)
    : interp_(interp) {
}

void SimdChebyshevInterpolationFloat::evaluate(
    ...
    // Process in groups of 8 using AVX2 SIMD (instead of 4 as in the double version)
    size_t i = 0;
    for (; i + 7 < x.size(); i += 8) {
        std::array<float, 8> x_block = {
...
std::array<float, 8> SimdChebyshevInterpolationFloat::evaluate8(
...
std::shared_ptr<ChebyshevInterpolationFloat> createChebyshevInterpolationFloat(
    ...
    return std::make_shared<ChebyshevInterpolationFloat>(
        num_points, func, kind, domain_start, domain_end);
}
